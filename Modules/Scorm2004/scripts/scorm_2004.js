...
function init (n) {
        ...
        if (!ScormApi) {
                ...
                say ('init', 'Continuing without  support.');
                ...
                lesson_mode   = getValue ('cmi.mode');
                lesson_status = getValue ('cmi.completion_status');
                if (lesson_status == 'not attempted' || lesson_status == 'unknown') {
                        setValue (
       ...
       /* with (document) {
                var type;
                for (n=0, i=0; n<forms.length; n++) {
                        if (forms[i].name.indexOf ('fi_') != 0) {
                                continue;
                        }
                        for (j=0; j<forms[i].length; j++) {
                                if (!forms[i][j]) continue;
                                switch (forms[i][j].type) {
                                case 'checkbox':
                                case 'radio':
                                        if (R[i].length == 1 && R[i][0] == 0) {
                                                type = 'likert';
                                        } else {
                                                type = 'choice';
                                        }
                                        forms[i][j].checked = false;
                                        break;
                                case 'textarea':
                                             type = 'performance';
                                        forms[i][j].value = '';
                                ...
                                case 'select-one':
                                        type = 'matching';
                                        forms[i][j].selectedIndex = 0;
                                }
                        }
                        forms[i].reset();
 setValue('cmi.interactions.'+i+'.id',i);
                        setValue('cmi.interactions.'+i+'.type',type);
                        i++;
                }
        }
...
function mydynShow (cont) {
        ...
        if (!dynShown.length) {
                ...
                setValue ('cmi.completion_status', lesson_status);
                hide ('continue');
                show("learningobjecttitle");
                finish();
...
function setLocation () {
        setValue ('cmi.location', dynSect+':'+dynStep+':'+dynForm);
}
...
function say (w, s) {
       /* if (document.layers) {
                with (getObj(w+'msg')) {
                        open();write(s+"<br>");close();
                }
        } else {
                getObj(w+'msg').innerHTML+=s+"<br>";
        }    */
}
...
function findScormApi (w) {
        ...
        try {
                if (w.API_1484_11) { // found it
                        ScormApi = w.API_1484_11;
						ScormApi.version = "1.3";
                        return;
                }
        } catch (e) {
                return;
...
function setValue (l, r) {
        say ('dbg', 'SetValue ("'+l+'","'+r+'");');
        if (!ScormApi) return;
        ScormApi.SetValue (l, r);

}
function getValue (l) {
        var r;
        if (ScormApi) {
                r = ScormApi.GetValue (l);
        }
        say ('dbg', 'GetValue ("'+l+'")="'+r+'"');
        return r;
}
...
function finish () {
        ...
        setElapsedTime ('cmi.session_time', initTime);
        ScormApi.Terminate('');
        ScormApi = null;
}
